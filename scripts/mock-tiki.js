...
async function fetchTikiProducts(categoryId) {
  const url = `https://tiki.vn/api/personalish/v1/blocks/listings?limit=25&category=${categoryId}&page=1`;
  try {
...
async function run() {
  ...
  for (const cat of TIKI_CATEGORY_IDS) {
    ...
    const products = await fetchTikiProducts(cat.id);
    ...
    for (const p of products) {
       ...
       const brandRes = await client.query('SELECT id FROM brands WHERE name = $1', [brandName]);
       ...
       try {
         ...
         `, [
           productId, dbCatId, dbBrandId, p.name, productSlug, 
           p.thumbnail_url, p.short_description || p.name, true
         ]);
       } catch (e) {
         continue;
       }
       ...
       try {
          ...
          `, [
            variantId, productId, sku, p.thumbnail_url, 
            p.price, p.original_price || p.price, p.stock_item?.qty || 100
          ]);
       } catch (e) {}
