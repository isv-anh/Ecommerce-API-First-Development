...
async function run() {
  ...
  for (const v of variants.rows) {
    const invRes = await client.query('SELECT 1 FROM warehouse_inventory WHERE warehouse_id = $1 AND product_variant_id = $2', [warehouseId, v.id]);
  ...
  for (const p of products.rows) {
    const existingImgs = await client.query('SELECT 1 FROM product_images WHERE product_id = $1', [p.id]);
    if (existingImgs.rowCount === 0) {
       ...
       // Insert dummy extra images for carousel
       const dummyImage2 = 'https://placehold.co/600x600?text=Product+Image+2';
       await client.query(`
         INSERT INTO product_images (id, product_id, url, is_thumbnail, sort_order, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW())
       `, [crypto.randomUUID(), p.id, dummyImage2, false, 2]);

       const dummyImage3 = 'https://placehold.co/600x600?text=Product+Image+3';
       await client.query(`
         INSERT INTO product_images (id, product_id, url, is_thumbnail, sort_order, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW())
